...
def extract_python_list(text):
    """
    Extract the first Python-like list from the text, even if wrapped in code block or markdown.
    """
...
def analyze_image(img: Image, dict_of_vars: dict):
    try:
        ...
        answers = []
        try:
            # Try to extract a Python-like list from the response
            cleaned_text = extract_python_list(response.text)
            answers = ast.literal_eval(cleaned_text)
        except Exception as e:
            print(f"Error in parsing response from Gemini API: {e}")
            # Try to parse as JSON after replacing single quotes with double quotes
            try:
                cleaned = extract_python_list(response.text).replace("'", '"')
                answers = json.loads(cleaned)
            except Exception as e2:
                print(f"Error in JSON parsing fallback: {e2}")
                answers = []
